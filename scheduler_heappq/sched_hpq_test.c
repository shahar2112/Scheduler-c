...
#include "sched_hpq.h" /* forward decleration */
...
int main()
{
	...
	uid = SchedAdd(scheduler, Action, 1, NULL);
	printf("uid of first task is %ld\n", uid.id);
	CheckUid(uid);
	
	uid2 = SchedAdd(scheduler, Action2, 2, NULL);
	printf("uid of second task is %ld\n", uid2.id);
	CheckUid(uid2);
	
	uid3 = SchedAdd(scheduler, StopAction, 5, scheduler);
...
/*************************************************************************/
int RemoveAction(void *params) /*the param will be the helper struct */
{
	sched_t *sched = ((check_t *)params)->scheduler;
	ilrd_uid_t uid = ((check_t *)params)->uid;
