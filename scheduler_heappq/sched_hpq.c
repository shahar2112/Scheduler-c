...
static int CmpFunc(void *current_task, void *new_task );
static void WaitToNextApperance(task_t *next_task);
static int IsMatch(void *list_data, void *param_from_user);
...
sched_t *SchedCreate()
{
	...
	if(NULL == scheduler)
	{
	 ...
	 scheduler->pr_q = PQCreate(CmpFunc);
...
static int CmpFunc(void *current_task, void *new_task )
{		 
	return TaskGetTime2Run((task_t*)current_task)  - TaskGetTime2Run((task_t*)new_task);
}
